...
occ_effect_sizes$group = addNA(occ_effect_sizes$group)
...
ggplot(occ_effect_sizes %>% filter(name == "bibi"),
  ...
  labs(title = "Community level effect sizes for occurrence covariates",
       subtitle = m, x = "Coefficient estimate", y = "Group")

plots = list()
for (param_name in unique(occ_effect_sizes$name)) {
  plots[[param_name]] = ggplot(occ_effect_sizes %>% filter(name == param_name, group != "other"),
...
(plots[["bibi"]] + labs(y = "Group") + theme(legend.position = "left")) +
(plots[["canopy_reach"]] + theme(legend.position = "none", axis.text.y = element_blank(), axis.ticks.y = element_blank())) +
(plots[["canopy_basin"]] + theme(legend.position = "none", axis.text.y = element_blank(), axis.ticks.y = element_blank())) +
(plots[["imp_reach"]] + theme(legend.position = "none", axis.text.y = element_blank(), axis.ticks.y = element_blank())) +
(plots[["imp_basin"]] + theme(legend.position = "none", axis.text.y = element_blank(), axis.ticks.y = element_blank()))
...
for (param_name in unique(det_effect_sizes$name)) {
  plots[[param_name]] = ggplot(det_effect_sizes %>% filter(name == param_name, group != "other"),
                               aes(x = mean, y = as.factor(model), group = as.factor(group), shape = as.factor(group), color = as.factor(overlap0) == 0)) +
    geom_vline(xintercept = 0, color = "gray") +
    geom_point(position = position_dodge(width = 0.5)) +
    geom_errorbar(aes(xmin = `25%`,  xmax = `75%`), width = 0, linewidth = 1, position = position_dodge(width = 0.5)) +
...
(plots[["yday"]] + labs(y = "Parameter") + theme(legend.position = "left")) +
  (plots[["canopy_reach"]] + theme(legend.position = "none", axis.text.y = element_blank(), axis.ticks.y = element_blank())) +
  (plots[["imp_reach"]] + theme(legend.position = "none", axis.text.y = element_blank(), axis.ticks.y = element_blank()))
...
param_name = "alpha1"
param_data = param_alpha_data[[m]] %>% filter(param == param_name)
...
coefs = model_data[[m]]$msom_summary %>% filter(stringr::str_starts(param, param_name)) %>% arrange(mean) %>% mutate(plot_order = 1:nrow(.)) %>%
...
mu_param_samples  = as.matrix(model_data[[m]]$msom$sims.list[[paste0("mu.", param_name)]])
...
p_bibi = ggplot(predictions %>% filter(group != "other"), aes(x = idx, y = psi, group = common_name)) +
